    ...
    protected void execute() {
        ...
        for (ScoredPair pair : scores) {
            ...
            for (Rule rc : completeRules) {
                if (rc.getSite() == pair.getR1().getSite()) {//não tem interseção não vazia (sobreposição) pq são de sites diferentes
                } else if (!util.intersection(rc.getPairsPageValue(), pair.getR1().getPairsPageValue()).isEmpty()) {
                    hasIntersection = true;
                    printLog("\t " + pair.getR1().getRuleID() + "(Site: " + pair.getR1().getSite() + ") has intersection with " + rc.getRuleID() + " (Site: " + rc.getSite() + ")", true);
                    break;
                }
                if (rc.getSite() == pair.getR2().getSite()) {//não tem interseção não vazia (sobreposição) pq são de sites diferentes
    ...
    private List<ScoredPair> scorePairs(List<Rule> rulesSite, List<Rule> rulesComparedSite) {
        ...
        for (int i = 0; i < rulesSite.size() - 1; i++) {
            for (int j = 0; j < rulesComparedSite.size(); j++) {
                //sempre a intersecção será fazia pois são de sites diferentes
